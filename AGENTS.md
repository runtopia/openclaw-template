...
- OpenClaw is installed via the Dockerfile `OPENCLAW_VERSION` build arg (currently `2026.7.1-2`)
...
8. **Repair assistant's gateway restart MUST be non-blocking** → `restart_gateway` tool and `POST /repair/restart` call `restartGateway({ waitReady: false })`. The default `restartGateway()` awaits the Gateway startup readiness timeout (180s, long enough for first-boot plugin preparation). If the repair assistant awaited readiness, a gateway that fails to start (the exact reason the user opened repair) would freeze the SSE chat then abort with an error. Non-blocking restart returns immediately; the AI then polls `get_status`/`read_logs`. The startup path (src/index.js `ensureGatewayRunning`) keeps the blocking default since it must confirm readiness before serving traffic and immediately reports a healthy OneClaw heartbeat once ready.
...
10. **Plugins are pre-baked into the image, NOT `npm install -g`** → Verified against the OpenClaw 2026.7.1 stable line (`discovery-*.js`): plugin discovery scans only bundled dir, `config/extensions`, `workspace/.openclaw/extensions`, `plugins.load.paths`, and managed install records — it does **NOT** scan global `node_modules`, so `npm install -g <plugin>` is invisible to OpenClaw. The old managed-install approach (`openclaw plugins install` → `$STATE_DIR/npm`) put plugins on the `/data` volume, which the mount shadows, forcing a ~650MB `cp` on every boot (~70s; see git 2740039). The Dockerfile installs exact locked packages into `/opt/openclaw-plugins`, outside the volume. Before Gateway launch, `src/config/plugin-install-index.js` synchronizes those packages into the canonical `state/openclaw.sqlite` installed-plugin index; only after that succeeds does the wrapper remove matching `/data/extensions` copies and managed npm projects. Do not replace this with legacy `openclaw.json.plugins.installs`, which 2026.7.1-2 does not treat as authoritative during startup Doctor convergence. Normal plugins are exposed through `plugins.load.paths`; only `oneclaw-channel` is copied into the immutable OpenClaw `dist/extensions` tree and removed from ordinary `/opt` discovery. Retired collaboration plugins are removed on startup. Employee orchestration uses native Agent Sessions, Background Tasks, Workboard, and `update_plan`.
11. **Channel and provider plugin facts (verified against openclaw 2026.7.1)** → **telegram** is built into openclaw core (`dist/extensions/telegram`) — no plugin to install. **slack/discord/feishu/whatsapp** are official standalone packages (`@openclaw/<name>`) and are pinned to `2026.7.1`. **feishu (飞书/Lark)** is `@openclaw/feishu`, channel id `feishu` (alias `lark`); earlier this repo used the third-party `@larksuite/openclaw-lark` (channel id `openclaw-lark`), which was replaced by official `@openclaw/feishu`. When bumping feishu, re-verify minHostVersion ≤ host with `npm view @openclaw/feishu@<v> openclaw.install.minHostVersion`. **wechat (微信)** has no official plugin; it uses `@tencent-weixin/openclaw-weixin@2.4.6`, whose channel id and plugin id are both `openclaw-weixin`. **clawrouters** is the official npm package `@oneclaw-plugins/clawrouters@0.4.1` (plugin id `clawrouters`), installed by the locked plugin bundle and loaded through `plugins.load.paths`.
...
13. **Runtime defaults are patched on every configured startup** → Fresh config generation and persisted-volume redeploys both run `applyRuntimeDefaults()` from `src/config/runtime-defaults.js`. It removes the retired `oneclaw-workflows` and `oneclaw-employee-catalog` registrations, enables image-bundled `oneclaw-channel`, Workboard, and `tools.experimental.planTool=true` while preserving explicit opt-outs. It forces `agents.defaults.heartbeat={every:"2h",target:"last"}` and, when ClawRouters credentials are present, enables `agents.defaults.memorySearch` with provider `clawrouters`, the stable versioned model profile `oneclaw-memory-v1`, sources `["memory","sessions"]`, and `remote.baseUrl` set to the ClawRouters `/api/v1` base. Persistent memory indexes must never use embedding `auto` or cross-model fallback. OpenClaw appends `/embeddings` internally, so do not write `/api/v1/embeddings` into config. The same patch enables the image-bundled `oneclaw-search` plugin, prepares env-SecretRef-backed `messages.tts` (`model:"tts"`) and native OpenAI `talk.realtime` (`model:"realtime"`, `transport:"gateway-relay"`, `brain:"agent-consult"`) through ClawRouters, and selects OneClaw Search only when no other search provider was selected. User-owned Talk/TTS/search providers and explicit opt-outs are preserved; removing the child key deletes only the OneClaw-managed voice entries from persisted config. Matching Desktop, the template pins official `openai` and `openai-codex` providers to the embedded `pi` runtime unless the user already selected a runtime. This prevents a multi-GB managed install on first cloud boot. OpenClaw 2026.7.1 hardcodes the native Realtime WebSocket host, so the Docker image applies `scripts/patch-openclaw-realtime-base-url.mjs` fail-closed against the exact pinned bundle; revalidate its anchors whenever bumping OpenClaw. Managed batch ASR is separate and must not be presented as the Control UI's streaming transcription provider.
...
16. **Cloud startup and skill bundle stay volume-free** → `start.sh` performs recursive ownership repair only once per volume (`.oneclaw-ownership-v1`), and subsequent boots only chown the three top-level directories. Common Desktop-aligned skills (`pdf`, `xlsx`, `docx`, `pptx`, `find-skills`, `self-improving-agent`) and the first-party `manage-message-channels` workflow live under `/opt/oneclaw-skills` and load through `skills.load.extraDirs`; external-application skills such as `composio-gmail` ship with `oneclaw-integrations` so Skill and Tool versions stay aligned. Do not copy skills into `/data` on startup or install them as a connection side effect. Managed/workspace skills have higher OpenClaw precedence and remain the user override path. The default `cloud` build enables the lightweight and first-party workflow skills and omits document/OCR dependencies, extended local agent CLIs, browser libraries, standalone ClawHub, and niche Go binaries. `ONECLAW_DOCUMENT_SKILLS=1` adds the broad local `pdf`/`xlsx`/`docx`/`pptx` document runtimes. The paid-Gemini-dependent `nano-pdf` remains full-only; runtime employee sync aliases legacy `nano-pdf` assignments to `pdf`. `ONECLAW_RUNTIME_PROFILE=full` restores the remaining extended toolchain.
...
18. **Personality is preloaded before Gateway spawn** → `src/index.js` first reuses the private last-successful personality cache on normal restarts; only a fresh volume performs `/runtime/personality` and MCP snapshot fetches with a strict 750ms bound. `prepareEmployeesForStartup()` batches all Agent config changes into one atomic patch, then writes `IDENTITY.md`, `SOUL.md`, user context, and managed memory locally and idempotently before `ensureGatewayRunning()`. Post-ready reconciliation refreshes live platform state. Do not restore `openclaw setup` (2026.7.1 requires an interactive TTY and wasted 9–25s), and do not replace local canonical workspace writes with duplicate `agents.files.set` RPCs. Heartbeat and Redis-woken command long polling start only after initial reconciliation so they cannot contend with startup config reload. OpenClaw startup tracing is enabled by default through `OPENCLAW_GATEWAY_STARTUP_TRACE=1`.
19. **Gateway fast boot is exact-version and fail-safe** → The image sets `ONECLAW_FAST_GATEWAY_ENTRY=/app/scripts/openclaw-gateway-fast.mjs`. For the pinned `2026.7.1-2` host it imports the single validated `runGatewayCommand` bundle directly, avoiding the generic CLI environment-selection and Doctor bootstrap that the wrapper already completed. Docker runs `--check` during the build. Any version/export/argument drift before direct startup automatically falls back to the public `openclaw gateway run` path; after direct startup begins, failures are left to `src/gateway/manager.js` crash recovery so a second process cannot race the lock or port. Revalidate and update `SUPPORTED_OPENCLAW_VERSION` whenever bumping OpenClaw.
20. **Unreleased OneClaw plugin updates use local content-addressed tgz files** → `develop` maps to the 101 test environment and `main` maps to formal releases. Channel source stays canonical in sibling `runtopia/oneclaw-plugins`; do not duplicate it here. Development changes merge into `oneclaw-plugins/develop`, then `npm run update:local-channel` packs the clean synchronized `develop` commit into Template `develop`. The archive name includes the package version plus its full SHA-256; never replace bytes behind an existing name. After 101 acceptance, prepare formal plugin metadata from synchronized plugin `develop` on a short release branch and merge it to plugin `main`. Merge Template Runtime source changes from `develop` to Template `main`, but keep `main`'s exact npm-backed `resources/openclaw-plugin-bundle` when generated bundle files conflict; do not hand-merge tgz or lockfile bytes. Finally `npm run release:oneclaw-plugin -- <plugin-id> --tag vX.Y.Z` publishes from clean synchronized plugin `main`, updates that production bundle to the new exact npm versions, and pushes the Template release tag. After release, merge both `main` branches back into their `develop` branches and rerun `npm run update:local-channel` on Template `develop`; regenerated test artifacts are authoritative for the next 101 cycle.
21. **Production OneClaw Channel transport is v2 only** → Set `ONECLAW_INTERNAL_CHANNEL_V2=1` and use `/api/v2/internal/channel/{connect,events,commands,ack}`. Command polling opts into API long polling with `wait_ms`; the API holds empty polls and wakes them through the Runtime Redis notification bus, while the plugin uploads events concurrently. Do not restore the v1 WebSocket fallback or the unversioned `/internal/channel/v2/*` path.
22. **Composio actions stay behind the Integration broker** → The managed `oneclaw-composio-main` MCP snapshot is authorization/catalog state, not a model tool-surface declaration. Do not register it in `openclaw.json.mcp.servers` or expose every `oneclaw-composio-main__*` action to the model. `oneclaw-integrations` injects a compact connected-app catalog; Agents use dedicated aggregate tools where available, otherwise `search_integrations` followed by `use_integration`. This keeps the model tool list bounded as integrations grow.
23. **OneClaw Cron creation must use the native tool with an explicit tool budget** → Authenticated OneClaw turns may access native `cron`, while `gateway` and `nodes` remain owner-only. Channel blocks `openclaw cron` through `exec` because it bypasses delivery binding. For every `agentTurn` Cron add, the Agent must set `payload.toolsAllow` to the smallest exact future-run tool set (`[]` when none); OpenClaw validates it against the creator surface, persists it, and enforces it during execution. Native tool-hook `channelId` may be `oneclaw:session_...` rather than the plain `oneclaw` provider ID; recognize both while keeping other providers out of this policy. Before native `cron.add` persists a job, OneClaw Channel resolves flat/wrapped/payload-shorthand inputs before OpenClaw's own argument recovery and normalizes creation to `sessionTarget:"isolated"`, `delivery:{mode:"announce",channel:"oneclaw",to:<trusted originating Session>}`. Model-authored `systemEvent` payloads become `agentTurn` with the event text and `toolsAllow:[]`; existing Agent payloads and explicit tool budgets are preserved. Creation without a trusted OneClaw Session is blocked. Other channels, administrative creation, updates, and existing persisted jobs retain their native behavior; do not add startup rewrites.
