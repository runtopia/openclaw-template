...
18. **Personality is preloaded before Gateway spawn** → `src/index.js` fetches `/runtime/personality` with a strict 750ms bound and calls `prepareEmployeesForStartup()` before `ensureGatewayRunning()`. Existing Agent config plus `IDENTITY.md`, `SOUL.md`, and managed memory are written locally and idempotently; post-ready reconciliation handles only remaining runtime work. Do not restore `openclaw setup` (2026.7.1 requires an interactive TTY and wasted 9–25s), and do not replace local canonical workspace writes with duplicate `agents.files.set` RPCs. Heartbeat and Redis-woken command long polling start only after initial reconciliation so they cannot contend with startup config reload. OpenClaw startup tracing is enabled by default through `OPENCLAW_GATEWAY_STARTUP_TRACE=1`.
