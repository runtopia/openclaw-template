...
- **Dockerfile** — 单阶段构建：安装 OpenClaw core、预装插件到 `/opt/openclaw-plugins`（编排插件同步内置到 OpenClaw `dist/extensions`）、安装 wrapper 依赖
...
10. **Plugins are pre-baked into the image, NOT `npm install -g`** → Verified against openclaw 2026.7.1 source (`discovery-*.js`): plugin discovery scans only bundled dir, `config/extensions`, `workspace/.openclaw/extensions`, `plugins.load.paths`, and managed install records — it does **NOT** scan global `node_modules`, so `npm install -g <plugin>` is invisible to OpenClaw. The old managed-install approach (`openclaw plugins install` → `$STATE_DIR/npm`) put plugins on the `/data` volume, which the mount shadows, forcing a ~650MB `cp` on every boot (~70s; see git 2740039). The Dockerfile now installs the exact versions declared by `resources/openclaw-plugin-bundle/package.json` and its lockfile into `/opt/openclaw-plugins` (env `OPENCLAW_PLUGINS_DIR`, **outside** the volume). Normal plugins are exposed through `plugins.load.paths`; `oneclaw-workflows` and `oneclaw-employee-catalog` are copied into the immutable OpenClaw `dist/extensions` tree, removed from the final `/opt` tree, and deliberately omitted from both `/opt` load paths and `plugins.installs`. Those two plugins call privileged Gateway APIs, which OpenClaw allows only for bundled or catalog-trusted official plugins; a surviving `/opt` config/global candidate (including one recovered from the SQLite installed-plugin index) would shadow the trusted bundled candidate. This preserves the host trust boundary while retaining exact npm locks, zero runtime copy, zero runtime npm install, and volume-mount safety. Plugin source remains in its own npm package; this repository is only the consumer. `buildChannelPlan` gates on `availableChannels` (always true) instead of `channels add --help` — that help text doesn't list a plugin-backed channel until it is enabled. Activation still needs `plugins.entries.<id>.enabled=true`; runtime defaults enable `oneclaw-workflows` and `oneclaw-employee-catalog`, auto-config enables clawrouters, and channel reconciliation enables configured channels.
