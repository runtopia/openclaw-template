...
20. **Unreleased OneClaw plugin updates use local content-addressed tgz files** → Do not publish an npm version merely to test or deploy an in-development plugin change. Build and test the plugin in `runtopia/oneclaw-plugins`, run `pnpm pack`, copy the resulting archive into `resources/openclaw-plugin-bundle/tarballs/`, and name it with the package version plus the archive's full SHA-256. Point the matching dependency in `resources/openclaw-plugin-bundle/package.json` at `file:tarballs/<archive>.tgz`, regenerate `package-lock.json`, and commit the tgz, manifest, lockfile, and contract tests together. Never replace the bytes behind an existing archive name; a changed artifact gets a new content-addressed filename so Docker cache invalidation is deterministic. The automated npm freshness updater must skip `file:` dependencies. Only after the plugin's formal release workflow has published and verified the package on npm may this repository replace the `file:` spec with the new exact npm version and remove the superseded local tgz. Creating or pushing the plugin release tag and publishing npm are separate release actions and must not be folded into a template plugin-update task unless the user explicitly requests them.
21. **Production OneClaw Channel transport stays on v1 until the public ingress exposes v2** → Keep the schema-v6 `@oneclaw-plugins/channel@0.1.23` content-addressed artifact, but set `ONECLAW_INTERNAL_CHANNEL_V2=0`. `www.oneclaw.net` currently forwards `/api/v1/*` to the Go API while `/internal/channel/v2/*` reaches the Next.js site and returns 404. Do not work around that by downgrading Channel to 0.1.20: Runtime volumes already opened by 0.1.23 have SQLite `user_version=6`, which 0.1.20 rejects. Re-enable v2 only after the real public ingress contract is tested end to end.
