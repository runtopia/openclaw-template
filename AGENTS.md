...
- OpenClaw is installed via the Dockerfile `OPENCLAW_VERSION` build arg (currently `2026.7.1`)
...
10. **Plugins are pre-baked into the image, NOT `npm install -g`** → Verified against openclaw 2026.7.1 source (`discovery-*.js`): plugin discovery scans only bundled dir, `config/extensions`, `workspace/.openclaw/extensions`, `plugins.load.paths`, and managed install records — it does **NOT** scan global `node_modules`, so `npm install -g <plugin>` is invisible to OpenClaw. The old managed-install approach (`openclaw plugins install` → `$STATE_DIR/npm`) put plugins on the `/data` volume, which the mount shadows, forcing a ~650MB `cp` on every boot (~70s; see git 2740039). The Dockerfile now installs the exact versions declared by `resources/openclaw-plugin-bundle/package.json` and its lockfile into `/opt/openclaw-plugins` (env `OPENCLAW_PLUGINS_DIR`, **outside** the volume). Normal plugins are exposed through `plugins.load.paths`; `oneclaw-channel`, `oneclaw-workflows`, and `oneclaw-employee-catalog` are copied into the immutable OpenClaw `dist/extensions` tree, removed from the final `/opt` tree, and deliberately omitted from both `/opt` load paths and `plugins.installs`. Those plugins call privileged Gateway APIs, which OpenClaw allows only for bundled or catalog-trusted official plugins; a surviving `/opt` config/global candidate (including one recovered from the SQLite installed-plugin index) would shadow the trusted bundled candidate. The bundled Channel explicitly links the global OpenClaw host and the locked top-level `@oneclaw-plugins/runtime-events`, `ajv`, and `ws` dependencies. This preserves the host trust boundary while retaining exact npm locks, zero runtime copy, zero runtime npm install, and volume-mount safety. Plugin source remains in its own npm package; this repository is only the consumer. `buildChannelPlan` gates on `availableChannels` (always true) instead of `channels add --help` — that help text doesn't list a plugin-backed channel until it is enabled. Activation still needs `plugins.entries.<id>.enabled=true`; runtime defaults enable `oneclaw-channel`, `oneclaw-workflows`, and `oneclaw-employee-catalog`, auto-config enables clawrouters, and channel reconciliation enables configured channels.
...
13. **Runtime defaults are patched on every configured startup** → Fresh config generation and persisted-volume redeploys both run `applyRuntimeDefaults()` from `src/config/runtime-defaults.js`. It enables the image-bundled `oneclaw-workflows` plugin and defaults `tools.experimental.planTool=true` while preserving an explicit `false`. It forces `agents.defaults.heartbeat={every:"2h",target:"last"}` and, when ClawRouters credentials are present, enables `agents.defaults.memorySearch` with provider `clawrouters`, model `auto`, sources `["memory","sessions"]`, and `remote.baseUrl` set to the ClawRouters `/api/v1` base. OpenClaw appends `/embeddings` internally, so do not write `/api/v1/embeddings` into config. The same patch enables the image-bundled `oneclaw-search` plugin and selects it at `tools.web.search` only when no other provider was selected; an explicit provider or `enabled=false` is preserved. OpenClaw 2026.7.1 implicitly routes official `openai` providers through the separately downloaded `@openclaw/codex` harness; matching Desktop, template pins `models.providers.openai` and `openai-codex` to the embedded `pi` runtime unless the user already selected a runtime. This prevents a multi-GB managed install on first cloud boot.
