...
23. **OneClaw Cron creation must use the native tool with an explicit tool budget** → Authenticated OneClaw turns may access native `cron`, while `gateway` and `nodes` remain owner-only. Channel blocks `openclaw cron` through `exec` because it bypasses delivery binding. For every `agentTurn` Cron add, the Agent must set `payload.toolsAllow` to the smallest exact future-run tool set (`[]` when none); OpenClaw validates it against the creator surface, persists it, and enforces it during execution.
