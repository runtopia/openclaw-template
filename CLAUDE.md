...
11. **Channel plugin facts (host + plugins currently pinned to openclaw 2026.6.10)** → **telegram** is built into openclaw core (`dist/extensions/telegram`) — no plugin to install. **slack/discord/feishu/whatsapp** are official standalone packages (`@openclaw/<name>`), all pinned to the same version as the host (`2026.6.10`). **feishu (飞书/Lark)** is `@openclaw/feishu`, channel id `feishu` (alias `lark`). **Historical gotcha (now resolved):** feishu carries `openclaw.install.minHostVersion: ">=2026.5.29"`; while the host sat at `2026.5.28` this forced a pin to `2026.5.27` (which declares `>=2026.4.25`) because anything ≥`2026.5.28` was skipped with `plugin requires OpenClaw >=2026.5.29 ... skipping load`. Now the host is `2026.6.10` (≥ `2026.5.29`), so feishu rides `@latest` (`2026.6.10`) fine. **When bumping feishu, always re-verify `minHostVersion ≤ host`** with `npm view @openclaw/feishu@<v> openclaw.install.minHostVersion`. Earlier this repo used the third-party `@larksuite/openclaw-lark` (channel id `openclaw-lark`); replaced by official `@openclaw/feishu` so the channel is named `feishu`. **wechat (微信)** has NO official plugin; it uses the third-party `@tencent-weixin/openclaw-weixin` (**semver, not date-versioned** — pinned to `2.4.6`, which requires openclaw `>=2026.5.12`), whose channel id AND plugin id are both `openclaw-weixin`. WeChat login is QR-based at runtime (`openclaw channels login --channel openclaw-weixin`), so the wizard only enables the channel; the user scans the QR via the repair console / logs afterward. The wizard's old "webchat" option was removed (it was the embeddable web widget, not WeChat). clawrouters is GitHub-only (`github:runtopia/clawrouters-plugin`, id `clawrouters`), not on npm — another reason it must come via `plugins.load.paths` rather than lazy install.

12. **QR channel binding is per-channel — WhatsApp and WeChat use two completely different paths** → The QR is NOT in gateway stdout during normal channel runs. **WhatsApp** (`@openclaw/whatsapp`, baileys): the plugin sets `printQRInTerminal:false` and exposes the QR only via the gateway WS RPC `web.login.start` → `{ qrDataUrl }` (a `data:image/png;base64,...` PNG). The Control UI itself uses this (frontend var `whatsappLoginMessage`). The wrapper forwards it over its persistent gateway WS: `gatewayRpc.rpcGateway(method, params)` (src/lib/gateway-rpc.js) issues a self-routed req and resolves the res, exposed as `POST /repair/whatsapp-login/start` and `/wait`. **WeChat** (`@tencent-weixin/openclaw-weixin`): no web RPC at all — the QR is printed to stdout ONLY by `openclaw channels login --channel openclaw-weixin`, as an ASCII block PLUS a raw URL (`https://liteapp.weixin.qq.com/q/<id>?qrcode=...&bot_type=3`). `src/lib/wechat-login.js` spawns that CLI as a single managed process, parses stdout for the qrUrl + a connected marker, exposed as `POST /repair/wechat-login/start` + `GET /repair/wechat-login`. The two paths don't conflict (web.login.start is single-provider but only WhatsApp registers it; WeChat uses the separate CLI). Verified against openclaw 2026.6.5 source + local gateway/WC test. oneclaw_web's `QrBindModal` calls these endpoints (whatsapp→render qrDataUrl as `<img>`; wechat→render qrUrl with a QR library). QR session credentials persist under `STATE_DIR/credentials/` (so the `start.sh` STATE_DIR-on-volume fix is what keeps a scanned binding alive across redeploys).
