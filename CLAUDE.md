...
10. **Plugins are pre-baked into the image, NOT `npm install -g`** → Plugin discovery does not scan global `node_modules`, so the Dockerfile installs exact locked packages under `/opt/openclaw-plugins`, outside `/data`, and exposes normal plugins through `plugins.load.paths`. Before Gateway launch, synchronize those packages into the canonical `state/openclaw.sqlite` installed-plugin index; only after success remove matching `/data/extensions` copies and managed npm projects. Legacy `openclaw.json.plugins.installs` is not authoritative for OpenClaw 2026.7.1-2 startup Doctor convergence. Only privileged `oneclaw-channel` is copied into OpenClaw's immutable bundled extension tree. Retired collaboration plugins are removed on startup; employee orchestration uses native Agent Sessions, Background Tasks, Workboard, and `update_plan`.
...
13. **Runtime defaults are patched on every configured startup** → Fresh config generation and persisted-volume redeploys both run `applyRuntimeDefaults()` from `src/config/runtime-defaults.js`. It removes retired collaboration plugin registrations, enables image-bundled `oneclaw-channel`, Workboard, and `tools.experimental.planTool=true` while preserving explicit opt-outs. It forces `agents.defaults.heartbeat={every:"2h",target:"last"}` and, when ClawRouters credentials are present, enables `agents.defaults.memorySearch` with provider `clawrouters`, the stable versioned model profile `oneclaw-memory-v1`, sources `["memory","sessions"]`, and `remote.baseUrl` set to the ClawRouters `/api/v1` base. Persistent memory indexes must never use embedding `auto` or cross-model fallback. The same patch enables `oneclaw-search` when no other search provider was selected. Matching Desktop, `openai` and `openai-codex` providers default to the embedded `pi` runtime so OpenClaw 2026.7.1-2 does not download the multi-GB `@openclaw/codex` harness during first boot; explicit runtime selections are preserved.
