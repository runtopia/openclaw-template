...
- **src/config/** — 配置生成：从 env 构建 `openclaw.json`（direct-config、auto-config）
...
2. **Gateway auth**: Bearer token (auto-generated or from `OPENCLAW_GATEWAY_TOKEN` env)
   - Token is auto-injected into proxied requests (src/proxy/proxy.js, `proxyReq` + `proxyReqWs` handlers)
...
- HTTP requests: via `proxy.on("proxyReq")` event handler (src/proxy/proxy.js)
- WebSocket upgrades: via `proxy.on("proxyReqWs")` event handler (src/proxy/proxy.js)
...
- Gateway: Remove `Authorization` header injection (src/proxy/proxy.js) and verify requests fail

### Debugging gateway startup

Check logs for:
- `[gateway] starting with command: ...` (src/gateway/gateway.js)
- `[gateway] ready at <endpoint>` (src/gateway/gateway.js)
- `[gateway] failed to become ready after 20000ms` (src/gateway/gateway.js)
...
3. Register in `src/config/direct-config.js` or `src/config/auto-config.js`
...
- OpenClaw is installed via the Dockerfile `OPENCLAW_VERSION` build arg (currently `2026.6.11`)
