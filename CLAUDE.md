...
13. **Runtime defaults are patched on every configured startup** → Fresh config generation and persisted-volume redeploys both run `applyRuntimeDefaults()` from `src/config/runtime-defaults.js`. It enables the image-bundled `oneclaw-workflows` plugin and defaults `tools.experimental.planTool=true` while preserving an explicit `false`. It forces `agents.defaults.heartbeat={every:"2h",target:"last"}` and, when ClawRouters credentials are present, enables `agents.defaults.memorySearch` with provider `clawrouters`, model `auto`, sources `["memory","sessions"]`, and `remote.baseUrl` set to the ClawRouters `/api/v1` base. OpenClaw appends `/embeddings` internally, so do not write `/api/v1/embeddings` into config. The same patch enables the image-bundled `oneclaw-search` plugin and selects it at `tools.web.search` only when no other provider was selected; an explicit provider or `enabled=false` is preserved. Matching Desktop, `openai` and `openai-codex` providers default to the embedded `pi` runtime so OpenClaw 2026.7.1 does not download the multi-GB `@openclaw/codex` harness during first boot; explicit runtime selections are preserved.

14. **Structured input uses a private loopback broker** → The wrapper injects an ephemeral broker URL/token only into the Gateway child. `POST /repair/interactions/input` requires the instance-secret Bearer specifically. Its locked API contract is `docs/interaction-input-contract.md`: optional `sessionKey`, idempotent `200` replay, `409` conflict, `404` unknown, `410` expired, and no `202`.
