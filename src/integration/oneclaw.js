...
function employeeSoulContent(payload) {
  ...
  return [
    ...
    "## 长任务与交付\n输出预算按单次模型调用计算，包含工具参数，不是整个任务的总额度。大文件先写可运行骨架，再用多个工具调用分块补充和验证，避免单次工具参数包含整个大型应用。进度说明和承诺写文件不算交付；继续执行已授权的工具，检查产物与测试结果后再结束。遇到输出截断或中断，先检查已保存文件和成功工具结果，缩小下一步操作后继续；不要执行不完整工具参数或盲目重放有副作用的操作。恢复或预算耗尽时，明确告知任务未完成、已保存内容和剩余工作。",
  ].filter(Boolean).join("\n\n");
}
