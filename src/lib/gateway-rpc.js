...
const STARTUP_RETRY_WARN_AFTER_MS = 30_000;
const STARTUP_RETRY_WARN_EVERY_MS = 30_000;

export function createGatewayRpc({ gatewayHost, gatewayPort, gatewayToken, basePath, startupRetryWarnAfterMs = STARTUP_RETRY_WARN_AFTER_MS }) {
  ...
  let startupRetryStartedAt = null;
  let startupRetryLastWarnAt = 0;
      ...
      if (frame.type === "res") {
        ...
        if (hubHandshakeId && frame.id === hubHandshakeId) {
          if (frame.ok) {
            ...
            reconnectAttempt = 0;
            startupRetryCount = 0;
            startupRetryStartedAt = null;
            startupRetryLastWarnAt = 0;
            ...
            lastHandshakeError = frame.error || null;
  ...
  function scheduleReconnect(delayOverrideMs = null) {
    ...
    if (isStartupRetry) recordStartupRetry();
  ...
  function recordStartupRetry() {
    const now = Date.now();
    startupRetryCount += 1;
    startupRetryStartedAt ??= now;
    const elapsedMs = now - startupRetryStartedAt;
    if (
      elapsedMs >= startupRetryWarnAfterMs &&
      now - startupRetryLastWarnAt >= STARTUP_RETRY_WARN_EVERY_MS
    ) {
      startupRetryLastWarnAt = now;
      console.error(`[gateway-rpc] startup sidecars still unavailable after ${Math.round(elapsedMs / 1000)}s; continuing to retry`);
    }
  }
  ...
  function restart() {
    ...
    startupRetryCount = 0; startupRetryStartedAt = null; startupRetryLastWarnAt = 0; lastHandshakeError = null;
    intentionalClose = false;
    connectToGateway();
  }
  ...
  function close() {
    ...
    startupRetryCount = 0; startupRetryStartedAt = null; startupRetryLastWarnAt = 0;
  ...
  return {
    ...
    getConnectionState: () => ({
      ...
      reconnectAttempt,
      startupRetrying: startupRetryCount > 0 && !gatewayConnected,
      startupRetryCount,
      startupRetryElapsedMs: startupRetryStartedAt ? Date.now() - startupRetryStartedAt : 0,
      lastHandshakeError,
    }),
    waitUntilConnected,
    rpcGateway,
