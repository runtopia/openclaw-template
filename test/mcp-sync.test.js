...
function fixture(t, config) {
  ...
  return {
    configPath,
    connectionHeaders: { "X-OneClaw-Sidecar-MCP-Token": "a".repeat(64) },
    connectionUrl: "http://127.0.0.1:8080/internal/mcp/composio",
    statePath,
  };
}
...
test("invalid or tampered snapshots do not modify config", (t) => {
  ...
  const invalidPathServers = [server({ url: "runtime/integrations/other" })];
  assert.throws(() => applyMcpSnapshot({ ...paths, snapshot: snapshot(1, invalidPathServers) }), /unsupported MCP proxy path/u);
